resource "azurerm_management_group_policy_assignment" "custom_pol_assign" {
  name                 = var.name
...
resource "azurerm_role_assignment" "role_assignment" {
  ...
  principal_id       = azurerm_management_group_policy_assignment.custom_pol_assign.identity[0].principal_id
}

resource "azurerm_management_group_policy_remediation" "pol_remediation" {
  name                 = "rem_${var.name}"
  management_group_id  = var.scope
  policy_assignment_id = azurerm_management_group_policy_assignment.custom_pol_assign.id
}
