...
resource "azuread_app_role_assignment" "sp_grant_role_consent" {
  ...
  principal_object_id = azuread_service_principal.wiz_for_azure.object_id
...
resource "azurerm_role_definition" "wiz_custom_role" {
  ...
  permissions {
    actions = concat(
      ...
      local.WIZ_CUSTOM_ROLE_DISK_ACTIONS,
    )
  }
...
# the following resource will not be created
...
resource "azurerm_role_assignment" "wiz_custom_role_assignment" {
  scope                = local.scope
  count                = local.scope_exists ? 1 : 0
  principal_id         = azuread_service_principal.wiz_for_azure.object_id
...
# (Optional) Serverless Scanning Role Assignment
resource "azurerm_role_assignment" "wiz_serverless_scanning_role_assignment" {
  scope                = local.scope
  count                = local.scope_exists && var.enable_serverless_scanning ? 1 : 0
  principal_id         = azuread_service_principal.wiz_for_azure.object_id
  role_definition_name = var.wiz_serverless_scanning_role_name
  depends_on = [
    time_sleep.wait_for_az_dataplane_custom_role
  ]
}
...
# this resource will not be created
# (Optional) Data Scanning Role Assignment
resource "azurerm_role_assignment" "wiz_data_scanning_role_assignment" {
  scope                = local.scope
  count                = local.scope_exists && var.enable_data_scanning ? 1 : 0
  principal_id         = azuread_service_principal.wiz_for_azure.object_id
  role_definition_name = var.wiz_data_scanning_role_name
...
# Azure Kubernetes Service Cluster User Role Assignment
...
# Azure Kubernetes Service RBAC Reader Role Assignment
...
# this resource will not be created
# (Optiona) Cognitive Services OpenAI User Role Assignment
...
# (Optional) Storage Blob Data Reader Role Assignment
