...
resource "azurerm_app_service_virtual_network_swift_connection" "app-service-out" {
  ...
  app_service_id = azurerm_windows_function_app.windows_function_app[each.key].id
...
module "diagnostic_management" {
  ...
  target_id     = azurerm_windows_function_app.windows_function_app[each.key].id
...
resource "azurerm_function_app_function" "windows_function" {
  for_each = {
    for key, value in var.windows_function_apps : key => value
